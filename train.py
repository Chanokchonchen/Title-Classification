...
def cross_validate(df,save_name,engine,special_features=False,method='split',model="svm",vector="tfidf",min_count=3,save=False,list_dict=['wl1','wl2','wl3','wl4']):
  with open(f"{REPORT_PATH}report_{save_name}.txt", "w") as text_file:
      ...
      for train, test in kfold.split(df['title_th'].values):
        ...
        if vector == 'tfidf':
          vectorizer = TfidfVectorizer(tokenizer=tokenizer,min_df=min_count)
        else:
          vectorizer = CountVectorizer(tokenizer=tokenizer,min_df=min_count)
        ...
        print(f"Fold {fold} Train features : {vector_df_train.shape[1]} , Test features : {vector_df_test.shape[1]}")
        if (special_features):
          if method == "substr":
...
def train_model(engine,pre_path=False,special_features=False,method='split',model="svm",vector="tfidf",cross=False,min_count=3,list_dict=['wl1','wl2','wl3','wl4']):
      ...
      if cross:
        cross_validate(df,save_name,engine,special_features,method,model,vector,min_count,False,list_dict)
      else:
...
train_model(engine=engine,pre_path=pre,special_features=special,method=method,model=model,vector=vector,cross=cross,min_count=min_count,list_dict=list_dict)
